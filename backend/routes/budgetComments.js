...
router.put('/admin/:commentId/moderate', verifyToken, verifyAdmin, async (req, res) => {
  try {
...
router.post('/:commentId/like', async (req, res) => {
  try {
    ...
    if (!comment.isApproved) {
      return res.status(403).json({ message: 'Cannot like unapproved comment' });
    }
...
router.get('/budget/:budgetId/stats', async (req, res) => {
  try {
    ...
    const stats = await BudgetComment.aggregate([
      { $match: { budgetId: new mongoose.Types.ObjectId(budgetId), isApproved: true } },
      {
        $group: {
          _id: '$type',
          count: { $sum: 1 },
    ...
    res.json({
      ...
      engagement: {
        ...
        avgLikesPerComment: totalComments > 0 ? 
          stats.reduce((sum, stat) => sum + stat.totalLikes, 0) / totalComments : 0
      }
    });
