...
import { publicAPI, commentAPI } from "../../services/api";
import {
  formatDate,
  getStatusColor,
  getPriorityColor,
...
const BudgetDetails = () => {
  ...
  // Filtered comments based on type
  const filteredComments =
    commentTypeFilter === "all"
      ? comments
      : comments.filter((c) => c.type === commentTypeFilter);
    ...
    try {
      ...
      if (
        ...
        budgetData.remainingAmount =
          budgetData.totalAmount - (budgetData.spentAmount || 0);
      }

      // Ensure utilizationRate is calculated if not present
      if (
        budgetData.utilizationRate === undefined ||
        budgetData.utilizationRate === null
      ) {
        budgetData.utilizationRate =
          budgetData.totalAmount > 0
            ? ((budgetData.spentAmount || 0) / budgetData.totalAmount) * 100
            : 0;
      }
  ...
  const fetchComments = async () => {
    try {
      ...
      const res = await commentAPI.getBudgetComments(id);
  ...
  return (
    ...
    >
      ...
      >
        <motion.div
          className="bg-surface p-6 rounded-lg shadow-sm border border-border"
          variants={fadeUp}
        >
            ...
            >
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-textPrimary">
                    {formatCurrency(dept.spentAmount)} /{" "}
      ...
      {/* Recent Transactions */}
      <motion.div
        className="bg-surface rounded-lg shadow-sm border border-border"
        variants={fadeUp}
      >
      ...
      {/* Public Feedback Section - always visible */}
      <motion.div className="space-y-6 mt-10" variants={fadeUp}>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Help improve government transparency by sharing your thoughts,
                  suggestions, and questions about this budget.
                </p>
              </div>
            </div>
          </div>
          {commentStats.totalComments > 0 && (
            <div className="mt-4 flex items-center space-x-6 text-sm text-gray-600">
              ...
              <span className="flex items-center space-x-1">
                ...
                <span>
                  {commentStats.byType?.find((t) => t._id === "suggestion")
                    ?.count || 0}{" "}
                  suggestions
                </span>
              </span>
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 mb-2">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <label className="text-sm text-textMuted mr-2">
              Filter by type:
            </label>
            <select
              value={commentTypeFilter}
              onChange={(e) => setCommentTypeFilter(e.target.value)}
              className="px-2 py-1 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent"
            >
            ...
            <button
              ...
              className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors flex items-center space-x-2 ml-2"
            >
              <ChatBubbleLeftRightIcon className="w-4 h-4" />
        ...
        <motion.div className="space-y-4" variants={fadeUp}>
          {commentsLoading ? (
            ...
            <div className="text-center py-8 text-textMuted">
              ...
              <p>No comments yet for this type.</p>
            </div>
          ) : (
